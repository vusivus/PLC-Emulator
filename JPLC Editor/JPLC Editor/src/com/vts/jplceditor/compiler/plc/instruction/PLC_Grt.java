 ...
 * @author Vusman
 */
public class PLC_Grt extends Pic18Mem implements PLCInstr {
   ...
   private void checkENO() {
    ...
    private void byteToByte(PLCOperand in1, PLCOperand in2) {
        ...
        /*7*/ banksel(STATUS);
        /*8*/ asm.add(new BitInstruction(Opcode.BTFSC, STATUS, C, bsr));     //if in2<in1 C=1;
    ...
    private void byteToInt(PLCOperand in1, PLCOperand in2) {
        ...
        /*6*/ banksel(STATUS);
        /*7*/ asm.add(new BitInstruction(Opcode.BTFSS, STATUS, N, bsr));    //Check if 0-in2H produces negative                
        ...
        /*12*/ banksel(input1);
        /*13*/ asm.add(new ByteInstruction(Opcode.SUBWF, input1, a, bsr));  //in1-in2L
        /*14*/ banksel(STATUS);
        /*15*/ asm.add(new BitInstruction(Opcode.BTFSC, STATUS, N, bsr));    //if negative, not greater
    ...
    private void intToByte(PLCOperand in1, PLCOperand in2) {
       int greater = (org + 18)*2;
        ...
        /*1*/ asm.add(new ControlInstruction(Opcode.GOTO, endif));          // Goto ENDIF if EN=0
        /*3*/ banksel(input1);
        /*4*/ asm.add(new ByteInstruction(Opcode.MOVF, input1 + 1, a, w)); //check if in1H>0
        /*5*/ asm.add(new LiteralInstruction(Opcode.SUBLW, 0));
        /*6*/ banksel(STATUS);
        /*7*/ asm.add(new BitInstruction(Opcode.BTFSC, STATUS, N, a));    //Check if 0-in1H produces negative                
        ...
        /*12*/ banksel(input1);
        /*13*/ asm.add(new ByteInstruction(Opcode.SUBWF, input1, a, w));  //in1-in2L
        /*14*/ banksel(STATUS);
        /*15*/ asm.add(new BitInstruction(Opcode.BTFSC, STATUS, N, a));    //if negative, not greater
        /*16*/ asm.add(new ControlInstruction(Opcode.GOTO, not_greater));


        //here is the greater address
        /*18*/ banksel(bitAdress);
        /*19*/ asm.add(new BitInstruction(Opcode.BSF, bitAdress, ENO, a));//if in1<in2 set ENO
        /*20*/ asm.add(new ControlInstruction(Opcode.GOTO, endif));


        //here is the not_greater address
        /*22*/ banksel(bitAdress);
        /*23*/ asm.add(new BitInstruction(Opcode.BCF, bitAdress, ENO, a));//if greater than zero set ENO
        /*24*/ asm.add(new ControlInstruction(Opcode.NOP));   //endif address
        wordCount = 25+count;
    }
    ...
    private void intToInt(PLCOperand in1, PLCOperand in2) {
        ...
        /*1*/ asm.add(new ControlInstruction(Opcode.GOTO, endif));          // Goto ENDIF if EN=0
        /*3*/ banksel(input2);
        /*4*/ asm.add(new ByteInstruction(Opcode.MOVF, input2 + 1, bsr, w)); //check if in1H>in2H
        /*5*/ banksel(input1);
        /*6*/ asm.add(new ByteInstruction(Opcode.SUBWF, input1+1, bsr, w));  //in1H-in2H
        /*7*/ banksel(STATUS);
        /*8*/ asm.add(new BitInstruction(Opcode.BTFSC, STATUS, N, bsr));    //Check if in1H-in2H produces negative                
        ...
        /*13*/ banksel(input1);
        /*14*/ asm.add(new ByteInstruction(Opcode.SUBWF, input1, bsr, w));  //in1L-in2L
        /*15*/ banksel(STATUS);
        /*16*/ asm.add(new BitInstruction(Opcode.BTFSS, STATUS, N, bsr));    //if negative, not less
        /*17*/ asm.add(new ControlInstruction(Opcode.GOTO, not_greater));


        //here is the greater address
        /*19*/ banksel(bitAdress);
        /*20*/ asm.add(new BitInstruction(Opcode.BSF, bitAdress, ENO, bsr));//if in1<in2 set ENO
        /*21*/ asm.add(new ControlInstruction(Opcode.GOTO, endif));


        //here is the not_greater address
        /*23*/ banksel(bitAdress);
        /*24*/ asm.add(new BitInstruction(Opcode.BCF, bitAdress, ENO, bsr));//if greater than zero set ENO
