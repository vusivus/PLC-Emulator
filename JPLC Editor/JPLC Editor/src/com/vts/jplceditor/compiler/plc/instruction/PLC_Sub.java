...
public class PLC_Sub extends Pic18Mem implements PLCInstr {
    ...
    public PLC_Sub(int org, List<PLCOperand> parameters, PLCOperand prevENO) {
    ...
    private void byteAndByte(PLCOperand in1, PLCOperand in2) {
        ...
        wordCount = 7;
        if (dest.getDataType() == PLCDataType.INT) {
            /*14*/ asm.add(new ByteInstruction(Opcode.CLRF, des + 1, bsr, false));
            wordCount += 1;
        }
    }
    ...
    private void byteAndInt(PLCOperand in1, PLCOperand in2) {
        ...
        if (dest.getDataType() == PLCDataType.INT) {
            /*14*/ banksel(input2);
            /*16*/ asm.add(new ByteInstruction(Opcode.MOVF, input2 + 1, bsr, w));
            /*18*/ asm.add(new ByteInstruction(Opcode.MOVWF, des + 1, bsr, f));
            wordCount += 3;
        }
    }
    ...
    private void intAndInt(PLCOperand in1, PLCOperand in2) {
        ...
        if (dest.getDataType() == PLCDataType.INT) {
            ...
            /*16*/ asm.add(new ByteInstruction(Opcode.MOVF, input2 + 1, bsr, w));
            /*18*/ banksel(input1 + 1);
            /*20*/ asm.add(new ByteInstruction(Opcode.SUBWFB, input1 + 1, bsr, w));
